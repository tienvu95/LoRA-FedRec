...
class FedMF_SGD(MF):
    def __init__(self, item_num, gmf_emb_size=16, user_num=1, rank=4, bidirect=False):
        super().__init__(user_num, item_num, gmf_emb_size)
        self.rank = rank
    ...
    def _set_state_from_splited_params(self, splited_params):
        ...
        state_dict = OrderedDict({k: v for k, v in params_dict})
        if self.bidirect:
            state_dict['embed_item_GMF.weight'] = svd_compress(state_dict['embed_item_GMF.weight'], self.rank)
        self.load_state_dict(state_dict, strict=True)
